...
public class JwtProvider implements IJwtProvider {
    ...
    /**
     * <p>First, we require public and private keys for RSA encryption and decryption. Hence, below is the tool to generate RSA key online. It generates RSA public key as well as the private key of size 512 bit, 1024 bit, 2048 bit, 3072 bit and 4096 bit with Base64 encoded.</p>
     * <p>By default, the private key is generated in PKCS#8 format and the public key is generated in X.509 format.</p>
     *
     * @param jwtPrivateKeyStr JWT Private Key
     * @param jwtPublicKeyStr  JWT Public Key
     */

    public JwtProvider(@Value("${authentication.jwt.private-key}") String jwtPrivateKeyStr, @Value("${authentication.jwt.public-key}") String jwtPublicKeyStr) {
    ...
    public String generateToken(UserPrincipal authentication) {
        ...
        return Jwts.builder().setSubject(authentication.getUsername()).claim("userId", authentication.getId()).claim("roles", authorities).setExpiration(new Date(System.currentTimeMillis() + JWT_EXPIRATION_IN_MS)).signWith(jwtPrivateKey, SignatureAlgorithm.RS512).compact();
    }
    ...
    public Authentication getAuthentication(HttpServletRequest request) {
        ...
        List<GrantedAuthority> authorities = Arrays.stream(claims.get("roles").toString().split(",")).map(role -> role.startsWith("ROLE_") ? role : "ROLE_" + role).map(SimpleGrantedAuthority::new).collect(Collectors.toList());
